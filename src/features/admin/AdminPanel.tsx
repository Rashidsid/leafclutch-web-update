...
type Tab = 'dashboard' | 'services' | 'service-editor' | 'testimonials' | 'images' | 'settings';
...
function NewServiceForm({ onCancel, onCreate }: { onCancel: () => void; onCreate: (service: NewAdminService) => void }) {
    ...
    <Modal title="Add a service" onClose={onCancel} wide>
      ...
      <form
        onSubmit={event => {
          ...
          onCreate({
            icon, title, label: label || title.toUpperCase(), heading: heading || title, description,
            heroImage, images: [], features: [], status,
          });
        }}
        className="space-y-4"
      >
        <Field label="Icon">
          <div className="flex flex-wrap gap-1.5">
            {ICON_CHOICES.map(choice => (
              <button key={choice} type="button" onClick={() => setIcon(choice)} className={`w-9 h-9 rounded-lg text-lg flex items-center justify-center border transition-colors ${icon === choice ? 'border-accent bg-secondary' : 'border-border hover:bg-secondary/60'}`}>{choice}</button>
            ))}
          </div>
        </Field>
        ...
        <Field label="Status">
          <select value={status} onChange={event => setStatus(event.target.value as NewAdminService['status'])} className="admin-input">
...
export default function AdminPanel() {
  const { services, testimonials, websiteImages, adminPassword, addService, syncContent } = useAdmin();
      ...
      <div className="min-h-screen bg-[#0F1729] hero-grid flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl p-8 w-full max-w-sm shadow-2xl">
          <div className="text-center mb-6">
            <div className="w-14 h-14 rounded-xl bg-linear-to-br from-cyan-400 to-[#072069] flex items-center justify-center text-white text-2xl mx-auto mb-4">🔐</div>
            <h1 className="text-xl font-bold text-foreground">Admin Panel</h1>
            <p className="text-muted-foreground text-sm mt-1">Leafclutch Technology</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="text-sm font-medium text-foreground block mb-1.5">Email</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@leafclutchtech.com" className="w-full px-4 py-3 border border-border rounded-xl text-sm focus:outline-none focus:border-accent" />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground block mb-1.5">Password</label>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter admin password" className="w-full px-4 py-3 border border-border rounded-xl text-sm focus:outline-none focus:border-accent" />
            </div>
            {error && <p className="text-red-500 text-xs">{error}</p>}
            <button type="submit" className="w-full btn-primary text-white font-semibold py-3 rounded-xl text-sm">Login</button>
          </form>
        </div>
      </div>
    );
  ...
  const goTab = (next: Tab) => { setTab(next); if (next !== 'service-editor') setEditingServiceId(null); };
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex">
      ...
      <aside className="w-64 shrink-0 min-h-screen bg-[#0F1729] text-white flex flex-col">
          ...
          {NAV.map(item => (
            <div key={item.id}>
              <button
                type="button"
                onClick={() => {
                  if (item.id === 'services') { setServicesExpanded(v => !v); goTab('services'); }
                  else goTab(item.id);
                }}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                  tab === item.id || (item.id === 'services' && tab === 'service-editor') ? 'bg-white/10 text-[#3BE3A0]' : 'text-gray-300 hover:bg-white/5 hover:text-white'
                }`}
              >
                <span>{item.icon}</span>
                {item.label}
                {item.id === 'services' && <span className="ml-auto text-[10px]">{servicesExpanded ? '▾' : '▸'}</span>}
              </button>
        ...
        <div className="p-4 m-3 rounded-2xl bg-white/5 border border-white/10">
          <p className="text-sm font-bold text-white">Keep Your Website Always Updated</p>
          <p className="text-xs text-gray-400 mt-1.5 leading-relaxed">Manage content, images and client stories with ease.</p>
        </div>
        <p className="text-[11px] text-gray-500 text-center pb-4">© {new Date().getFullYear()} Leafclutch Technology · Admin Panel</p>
      </aside>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="bg-white border-b border-border px-6 py-3.5 flex items-center justify-between gap-4">
            ...
            {searchResults.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-border rounded-xl shadow-lg overflow-hidden z-30">
                {searchResults.map(result => (
                  <button key={result.key} type="button" onClick={result.onSelect} className="w-full text-left px-4 py-2.5 hover:bg-secondary transition-colors border-b border-[#F3F4F7] last:border-0">
          ...
          <div className="flex items-center gap-4 shrink-0">
            <Link href="/" className="text-muted-foreground hover:text-accent text-sm transition-colors">← Back to Site</Link>
            <div className="w-9 h-9 rounded-full bg-linear-to-br from-cyan-400 to-[#072069] flex items-center justify-center text-white text-sm font-bold">A</div>
            <button onClick={async () => {
              await supabase.auth.signOut();
              setAuthenticated(false);
            }} className="text-muted-foreground hover:text-red-500 text-sm transition-colors">Logout</button>
          </div>
        </header>

        <main className="flex-1 p-6 max-w-[1400px] w-full">
          {tab === 'dashboard' && <DashboardPanel onNavigate={next => goTab(next === 'services' ? 'services' : next === 'testimonials' ? 'testimonials' : 'images')} />}
          {tab === 'services' && <ServicesPanel onEdit={id => { setEditingServiceId(id); setTab('service-editor'); }} onAddNew={() => setNewServiceOpen(true)} />}
          {tab === 'service-editor' && editingServiceId && <ServiceEditorPanel serviceId={editingServiceId} onBack={() => goTab('services')} />}
